...
\begin{umlpackage}[y=-4.5]{LDP}
  ...
  \begin{umlpackage}[x=6,y=-4]{Operations}
    \umlinterface[]{OperationHandler}{}{
      + canHandle(ParsedRequest) : Promise<boolean>\\
      + handle(ParsedRequest) : Promise<void>\\
    }
    \umluniaggreg[mult=1,geometry=|-]{LdpHandler}{OperationHandler}

    \umlsimpleclass[right=1 of OperationHandler.north east,anchor=north west]{CompositeOperationHandler}
    ...
    \umlsimpleclass[below=.25 of CompositeOperationHandler.south west,anchor=north west]{GetOperationHandler}
